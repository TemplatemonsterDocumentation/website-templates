...
<h6>Intense используют расширение <b>RD Flickrgallery</b> для получения данных с сервиса <a
...
<p>    Вы можете вывести неограниченное количество элементов или использовать данный виджет несколько раз на странице.
    Так же, данное расширенное можно интегрировать с любым сторонним скриптом (например: карусель, галерея и т.д.)  HTML структура внутри виджета произвольная. Например, Вы можете использовать
    ТМ Grid систему для удобного расположения элементов. Обязательным является только наличие элемента с атрибутом <b>data-type="flickr-item"</b>.</p>

<h4>Настройка профиля пользователя Flickr</h4>
<p>Для того чтобы указать пользователя, необходимо указать идентификатор пользователя сервиса в атрибуте <b>data-id</b>. Например:</p>
<pre><code>
        &lt;div class="flickr" data-flickr-id="47302490@N05"&gt;
    </code></pre>
<p>Идентфикатор пользователя сервиса можно получить по следующей ссылке:</p>
<p><a href="http://idgettr.com/">http://idgettr.com/.</a></p>
<p>Для того чтобы получить данные с нескольких профилей пользователей, укажите их в атрибуте  <b>data-id</b> через запятую. </p>
<h4>Настройка фильтрации данных</h4>

<p>Для того чтобы отфильтровать данные по определенным тегам, необходимо добавить атрибут <b>data-tags</b>. Для указания нескольких тегов, введите их через запятую. Например:
</p>
...
<p>Для изменения языка содержимого, укажите атрибут <b>data-lang</b>. Например:
</p>
...
<p>Для получение данных об элементе виджета необходимо указать следующий атрибут: </p>
...
<p>С помощью суффикса картинки Вы можете указать желаемый размер полученной картинки. Доступны следующие значение:</p>
...
<p>Для получения ссылки на картинку сервиса необходимо указать атрибут <b>data-link="href,text"</b>. Например, для вывода ссылки с текстом ссылки, используйте следующий код:</p>

<pre><code>
        &lt;a href="#" data-link="href,text"&gt;&lt;/a&gt;
    </code></pre>

<h4>Отображение автора элемента</h4>

<p>Для отображения автора элемента с указанием адреса электронной почты, необходимо указать атрибут <b>data-author_name="text"</b>. Например:
</p>
<pre><code>
        &lt;p data-author_name="text"&gt;&lt;/p&gt;
    </code></pre>

<p>Для отображения автора элемента без указания адреса электронной почты, необходимо указать атрибут <b>data-author="text"</b>. Например:</p>
...
<h4>Отображение названия элемента
</h4>

<p>Для отображения названия элемента необходимо указать атрибут  <b>data-title="text"</b>. Например:</p>
...
<p>Для отображения даты добавления элемента необходимо указать атрибут  <b>data-dating="text"</b>. Что отобразить  дату в формате <b>YY-MM-DD</b>, необходимо указать атрибут <b>data-datetime="text"</b>.Например:</p>
<pre><code>
        &lt;time data-dating="text" data-datetime="datetime"&gt;&lt;/time&gt;
    </code></pre>

<h4>Отображение описания элемента</h4>
<p>Для отображения описания элемента необходимо указать атрибут <b>data-description="text"</b>. Например:
</p>
<pre><code>
