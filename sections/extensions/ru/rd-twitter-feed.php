...
<p>Intense использует расширение <b>jquery.rd-twitter-feed.js</b> для получения данных с сервиса <a
...
<p>Вы можете отобразить неограниченное количество элементов или использовать данный виджет несколько раз на странице.
    ...
    HTML структура внутри виджета произвольная. Например, Вы можете использовать ТМ Grid систему для удобного расположения элементов.
    Обязательно только наличие элемента с атрибутом <b>data-twitter-type="tweet"</b>.
</p>

<h4>Настройка проекта для работы с Twitter API</h4>

<p>Для работы с <b>Twitter API</b> Вам необходимо создать приложение со своего профиля пользователя Twitter, создать ключи и записать их в файл <b>bat/twitter_api/config.php</b>. Список необходимых ключей:</p>
...
<p>При запуске проекта с файловой системы плагин работать не будет, так как для работы плагина необходим PHP версии 5.2+.
    При использовании на локальном сервере, данные с сервиса Twitter не будут получены, так как сервис аутентификации Твиттера не может получить доступ к локальным данным.
    Вместо этого, будут показаны тестовые данные, идентичные полученным из Твиттера. При загрузке проекта на рабочий сервер будут получены реальные данные.</p>

<h4>Настройка профиля пользователя Twitter</h4>

<p>Для того чтобы указать пользователя, необходимо записать имя пользователя сервиса в атрибут <b>data-twitter-username</b>. Например:</p>
...
<p>Для того чтобы отфильтровать твиты по списку, необходимо записать его название в атрибут <b>data-twitter-listname</b> и указать имя пользователя при помощи атрибута <b>data-twitter-username</b>. Например:</p>
<pre><code>
...
<p>Отобразить данные элемента возможно только внутри блока с атрибутом <b>data-type="tweet"</b>.  Например:</p>
...
<h4>Отображение картинки твита</h4>
<p>
    Для отображения картинки твита  необходимо указать атрибут <b>data-media_url="src"</b>. Например:
</p>
<pre><code>
        &lt;img data-media_url="src" src="#" alt=""/&gt;
    </code></pre>

<p class="alert alert-info">Обратите внимание, если в твите отсутсвует картинка, данный элемент будет удален с разметки. Если Вы хотите вывести 2 или больше картинок продублируйте данный код необходимое количество раз.</p>

<h4>Отображение текста твита</h4>
<p>
    Для отображения текста твита необходимо указать атрибут  <b>data-tweet="text"</b>. Например:
</p>
<pre><code>
        &lt;p data-tweet="text"&gt;&lt;/p&gt;
    </code></pre>

<h4>Отображение даты добавления твита</h4>
<p>Для отображения даты добавления необходимо указать атрибут  <b>data-date="text"</b>. Например:</p>
<pre><code>
        &lt;div data-date="text"&gt;&lt;/div&gt;
    </code></pre>
<p>Для отображения даты в формате атрибута datetime тега time необходимо указать атрибут  <b>data-datetime="datetime"</b>. </p>

<h4>Отображение аватарки пользователя</h4>
<p>Для получения тегов аватарки необходимо указать атрибут <b>data-avatar="src"</b>. Например:</p>
<pre><code>
        &lt;img data-avatar="src" src="#" alt=""/&gt;
    </code></pre>
<h4>Отображение ссылки на твит</h4>
<p>Для получения названия элемента необходимо указать атрибут <b>data-title="text"</b>. Например:</p>

<pre><code>
        &lt;a data-url="href,text"&gt;&lt;/a&gt;
    </code></pre>
<h4>Отображение логина пользователя</h4>
<p>Для отображения логина пользователя в формате <b>"@templatemonster"</b> необходимо указать атрибут <b>data-screen_name="text"</b>. Например:</p>
