...
<p>Вы можете указать фильтр, который будет указывать на каких страницах производить поиск(по названию).
  Например, для выполнения поиска по HTML страницам, в названиях которых присутсвует слово <b>blog</b> фильтр выглядит следующим образом:
</p>
...
<h4>Настройка "живого" поиска</h4>
<p>Вы можете включить опцию “живого” поиска для отображения результатов без перезагрузки страницы. Для этого необходимо добавить атрибут <b>data-search-live</b> с указанием <b>id</b> элемента, в который будет выведен результат. Например:
</p>
<pre><code>
...
<h4>Настройка шаблона результатов поиска</h4>
<p>Шаблон результатов поиска тесно связан с настройкой страницы, описанной выше, а именно, с указанием различных мета тегов.
  ...
  в котором нужно указать соответствующую разметку для вывода результатов поиска. Строка поддерживает передачу параметров в формате <b>#{rule}</b>,
...
<p>Шаблон результата поиска по-умолчанию можно изменить в файле <b>script.js</b>  в переменной <b>defaultTemplate</b>, которая находится в коде после следующего комментария:</p>
